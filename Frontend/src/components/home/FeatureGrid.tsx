...
const features = [
  {
    ...
    badgeClassName: 'bg-brand-700 text-white',
  },
  {
    icon: Wallet,
    ...
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {features.map(({ icon: Icon, title, description, badgeClassName }, index) => (
        <div
          ...
          className="group animate-fade-in-up rounded-[26px] bg-[#f6f4ee] p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-lg"
        >
          <div className="flex items-start justify-between">
            ...
            <ArrowUpRight className="h-4 w-4 -translate-x-1 translate-y-1 text-gray-300 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:translate-y-0 group-hover:text-[#1c8f53] group-hover:opacity-100" />
          </div>
          <h3 className="mt-4 font-semibold text-[#1d3d2d]">{title}</h3>
