  ...
  return (
    <div className="py-3">
      ...
      <div className="flex items-center gap-3 sm:hidden">
        ...
        <p className="min-w-0 flex-1 truncate text-sm font-medium text-gray-900 dark:text-text-primary">{categoryName}</p>
        <span className="shrink-0 text-sm font-semibold text-gray-700 dark:text-text-secondary">{utilization}%</span>
      </div>
      ...
      <div className="hidden items-center gap-4 sm:flex">
        ...
        <div className="w-28 shrink-0 sm:w-36">
          <p className="text-sm font-medium text-gray-900 dark:text-text-primary">{categoryName}</p>
          <p className="text-xs text-gray-500 dark:text-text-muted">Spent / Budget</p>
        </div>
        ...
        <div className="w-32 shrink-0 text-right text-sm sm:w-36">
          <span className="font-medium text-gray-900 dark:text-text-primary">{formatCurrency(budget.spentAmount, DEFAULT_CURRENCY)}</span>
          <span className="text-gray-400 dark:text-text-muted"> / {formatCurrency(budget.limitAmount, DEFAULT_CURRENCY)}</span>
        </div>
        <div className="w-10 shrink-0 text-right text-sm font-semibold text-gray-700 dark:text-text-secondary">{utilization}%</div>
      </div>
    </div>
  );
}
