...
export function PublicLayout() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#dfeee3] dark:bg-background">
      <motion.header
        ...
        <div className="mx-auto max-w-[1400px]">
          <div
            className={cn(
              ...
              isScrolled
                ? 'border-black/[0.06] bg-white/75 px-4 py-1.5 shadow-lg shadow-black/5 backdrop-blur-xl dark:border-white/[0.06] dark:bg-surface/80 dark:shadow-black/30 sm:px-6'
          ...
          >
            <Link to={PUBLIC_ROUTES.home} className="group relative shrink-0 text-brand-900">
              <motion.div whileHover={{ scale: 1.05, rotate: -2 }} transition={{ type: 'spring', stiffness: 300 }}>
                <Logo
                  showTagline
                  wordmarkClassName="text-lg font-extrabold tracking-tight text-brand-700 dark:text-primary-accent sm:text-xl"
                  taglineClassName="dark:text-text-muted"
                />
              </motion.div>
            </Link>

            <nav className="hidden items-center gap-1 rounded-full border border-black/5 bg-white/70 p-1.5 text-sm font-medium text-gray-700 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5 dark:text-text-secondary lg:flex">
              {navLinks.map((link) => (
                  ...
                  {({ isActive }) => (
                    <span
                      className={cn(
                        ...
                        isActive ? 'text-white' : 'hover:text-[#1c8f53] dark:hover:text-primary-accent',
                      )}
                    >
                      {isActive && (
                        <motion.span
            ...
            <div className="hidden shrink-0 items-center gap-4 lg:flex">
              ...
              <ThemeToggle className={cn(!isScrolled && 'bg-white/70 shadow-sm backdrop-blur-sm dark:bg-white/5')} />
            </div>
            ...
            <div className="flex shrink-0 items-center gap-2 lg:hidden">
              ...
              <motion.button
                ...
                className={cn(
                  'flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-gray-700 transition-colors duration-200 hover:bg-black/5 dark:text-text-secondary dark:hover:bg-white/10',
                  !isScrolled && 'bg-white/70 shadow-sm backdrop-blur-sm dark:bg-white/5',
                )}
          ...
          >
            <div className="overflow-hidden rounded-3xl bg-white shadow-sm dark:bg-surface-elevated dark:shadow-black/30">
              <nav className="flex flex-col divide-y divide-gray-100 px-2 py-2 dark:divide-white/10">
                {navLinks.map((link) => (
                  <NavLink
                    key={link.to}
                    to={link.to}
                    ...
                    className={({ isActive }) =>
                      cn(
                        'rounded-2xl px-4 py-3 text-sm font-medium text-gray-700 transition-colors duration-200 hover:bg-gray-50 dark:text-text-secondary dark:hover:bg-white/5',
                        isActive && 'font-semibold text-[#1c8f53] dark:text-primary-accent',
                      )
                    }
                  >
                    {link.label}
