...
export function BudgetsPage() {
  ...
  const expenseCategories = useMemo(
    () => (user ? categoryService.list(user.id, 'expense') : []),
    [user],
  );
  const budgets = useMemo(
    () => (user ? budgetService.list(user.id, month) : []),
  ...
  function handleDelete(budget: Budget) {
    ...
    const confirmed = window.confirm(
      `Remove the ${categoryNameFor(budget.categoryId)} budget for ${displayMonth}?`,
    );
    if (!confirmed) return;
    budgetService.remove(user.id, budget.id);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (!parsedAmount || parsedAmount <= 0) {
      setError('Enter a limit amount greater than zero.');
      return;
    }
    ...
    try {
      if (editingBudget) {
        budgetService.update(user.id, editingBudget.id, parsedAmount);
        setNotice(`Updated the ${categoryNameFor(editingBudget.categoryId)} budget.`);
      } else {
        budgetService.create(user.id, { categoryId, month, limitAmount: parsedAmount });
        ...
        <Card className="animate-fade-in-up p-5">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div className="flex-1">
              <label htmlFor="budget-category" className="text-sm font-medium text-gray-700">
                Category
              </label>
              <select
                id="budget-category"
            ...
            <div className="flex-1">
              <label htmlFor="budget-limit" className="text-sm font-medium text-gray-700">
                Monthly limit
              </label>
          ...
          <EmptyState
            ...
            description={
              expenseCategories.length === 0
                ...
                : 'Set a monthly limit for a category to start tracking your spending against it.'
            }
          />
        </Card>
      ) : (
              ...
              <div key={budget.id} className="group flex items-center gap-2">
                ...
                <div className="flex shrink-0 gap-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                  <button
                    ...
                    className="rounded-lg p-1.5 text-gray-300 hover:bg-gray-100 hover:text-gray-700"
