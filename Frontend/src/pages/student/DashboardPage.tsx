      ...
      <div className="space-y-6">
        ...
        {setupIncomplete ? (
          <Card className="p-10">
            <EmptyState
              icon={Wand2}
              title="Your Campus Coin setup isn't complete yet"
              description="Complete your profile to get more useful budgeting insights and recommendations."
              action={
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-text-secondary">Here&apos;s your money overview for {formatMonthLabel(month)}.</p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            to={`${STUDENT_ROUTES.newTransaction}?type=expense`}
            className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-brand-700 hover:shadow-md active:translate-y-0 dark:bg-primary dark:hover:bg-primary-accent"
          >
            <Plus className="h-4 w-4" />
            Add Expense
          </Link>
      ...
      {setupIncomplete && (
        <Card className="flex flex-wrap items-center gap-3 border-brand-200 bg-brand-50 p-4 dark:border-primary-accent/20 dark:bg-primary-accent/10">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 text-brand-700 dark:bg-white/10 dark:text-primary-accent">
            <Wand2 className="h-4 w-4" />
          </span>
      ...
      {unreadNotifications.length > 0 && (
        <Card className="flex flex-wrap items-center gap-3 border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-400/10">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-600 dark:bg-amber-400/15 dark:text-amber-400">
          ...
          <Link to={STUDENT_ROUTES.notifications} className="shrink-0 text-sm font-semibold text-amber-700 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card className="p-5">
          <div className="flex items-center gap-2 text-gray-500 dark:text-text-secondary">
            ...
            <h2 className="text-sm font-semibold text-gray-900 dark:text-text-primary">Financial Health Snapshot</h2>
          </div>
          <p className="mt-3 text-sm text-gray-500 dark:text-text-secondary">
            {report
              ? report.netSavings >= 0
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2 text-gray-500 dark:text-text-secondary">
            ...
            <h2 className="text-sm font-semibold text-gray-900 dark:text-text-primary">Savings Goal Progress</h2>
          </div>
          {!user.savingsGoalAmount ? (
            <div className="mt-2">
              <p className="text-sm text-gray-500 dark:text-text-secondary">You haven&apos;t set a savings goal yet.</p>
              <Link to={STUDENT_ROUTES.profile} className="mt-2 inline-block text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-primary-accent dark:hover:text-primary">
            ...
            <>
              <div className="mt-3 flex items-end justify-between">
                <p className="text-lg font-bold text-gray-900 dark:text-text-primary">
                  ...
                  <span className="text-sm font-normal text-gray-400 dark:text-text-muted"> / {formatCurrency(user.savingsGoalAmount, DEFAULT_CURRENCY)}</span>
                </p>
                <span className="text-sm font-semibold text-brand-600 dark:text-primary-accent">
              ...
              <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
                <div
                  className="h-full rounded-full bg-brand-500 transition-all duration-500 dark:bg-primary-accent"
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            {categoryBreakdown.length === 0 ? (
              <p className="mt-3 text-sm text-gray-500 dark:text-text-secondary">No spending logged this month yet.</p>
            ) : (
              <>
                <p className="mt-3 text-lg font-bold text-gray-900 dark:text-text-primary">{categoryBreakdown[0].categoryName}</p>
          ...
          <Card className="p-5">
            ...
            {!topBudget || !budgetSummary ? (
              <div className="mt-2">
                <p className="text-sm text-gray-500 dark:text-text-secondary">No budgets set for this month yet.</p>
                <Link to={STUDENT_ROUTES.budgets} className="mt-2 inline-block text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-primary-accent dark:hover:text-primary">
                  Set a budget →
                </Link>
              </div>
            ) : (
              <>
                <p className="mt-1 text-xs text-gray-500 dark:text-text-muted">{topBudgetCategoryName}</p>
                <div className="mt-3 flex items-end justify-between">
                  <p className="text-lg font-bold text-gray-900 dark:text-text-primary">
                    {formatCurrency(topBudget.spentAmount, DEFAULT_CURRENCY)}
                    <span className="text-sm font-normal text-gray-400 dark:text-text-muted"> / {formatCurrency(topBudget.limitAmount, DEFAULT_CURRENCY)}</span>
                  </p>
                  <span
                    className={cn(
      ...
      {insight && (
        <Card className="flex items-start gap-3 border-blue-200 bg-blue-50 p-5 dark:border-blue-400/20 dark:bg-blue-400/10">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-100 text-blue-600 dark:bg-blue-400/15 dark:text-blue-400">
            <Sparkles className="h-4 w-4" />
          </span>
          <div>
            <p className="text-sm font-semibold text-gray-900 dark:text-text-primary">{insight.title}</p>
            <p className="mt-1 text-sm text-gray-600 dark:text-text-secondary">{insight.body}</p>
          </div>
        </Card>
      )}

      {topTip && (
        <Card className="flex flex-col gap-4 border-brand-200 bg-brand-50 p-5 dark:border-primary-accent/20 dark:bg-primary-accent/10 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-text-secondary">{topTip.body}</p>
            </div>
          </div>
          <Link to={STUDENT_ROUTES.savingTips} className="shrink-0 text-sm font-semibold text-brand-700 hover:text-brand-800 dark:text-primary-accent dark:hover:text-primary">
            View Details →
          </Link>
        </Card>
      )}
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between">
          ...
          <Link to={STUDENT_ROUTES.transactions} className="text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-primary-accent dark:hover:text-primary">
            View all
          </Link>
        </div>
          ...
          {transactions.slice(0, 5).map((txn) => {
            ...
            return (
              <Link
                ...
                className="flex items-center justify-between gap-3 py-3 transition-colors duration-150 hover:bg-gray-50 dark:hover:bg-white/5"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-text-primary">{txn.description || categoryName}</p>
                    <p className="text-xs text-gray-500 dark:text-text-muted">
                      {txn.type === 'income' ? 'Income' : categoryName} · {formatDate(txn.occurredAt)}
                    </p>
                  </div>
                </div>
                <span className={cn('text-sm font-semibold', txn.type === 'income' ? 'text-brand-600 dark:text-primary-accent' : 'text-gray-900 dark:text-text-primary')}>
                  {txn.type === 'income' ? '+' : '-'}
