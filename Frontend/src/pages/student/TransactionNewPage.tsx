  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <Card className="p-6 md:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input id="description" type="text" placeholder={type === 'income' ? 'e.g. Monthly allowance from parents' : 'e.g. Campus Cafe'} value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-border dark:bg-surface dark:text-text-primary dark:focus:border-primary-accent dark:focus:ring-primary-accent" />
            </div>
        ...
        <Card className="p-6">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-text-primary">Quick Categories</h2>
          <div className="mt-3 space-y-2">
            {categories.map((c) => {
              const quick = quickCategories[c.name];
              const Icon = quick?.icon ?? MoreHorizontal;
              return (
                <button key={c.id} type="button" onClick={() => setCategoryId(c.id)} className={cn('flex w-full items-center gap-3 rounded-lg border px-3 py-2 text-left text-sm font-medium transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm', categoryId === c.id ? 'border-brand-500 bg-brand-50 text-brand-700 dark:border-primary-accent dark:bg-primary-accent/10 dark:text-primary-accent' : 'border-gray-200 text-gray-700 hover:border-gray-300 dark:border-border dark:text-text-secondary dark:hover:border-white/20')}>
                  <span className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full', quick?.badgeClassName ?? 'bg-gray-100 text-gray-600 dark:bg-white/10 dark:text-text-secondary')}><Icon className="h-4 w-4" /></span>
                  {c.name}
                </button>
              );
            })}
            <Link to={STUDENT_ROUTES.categories} className="flex w-full items-center gap-2 rounded-lg border border-dashed border-gray-300 px-3 py-2 text-left text-sm font-medium text-gray-500 transition-colors duration-200 hover:border-brand-400 hover:text-brand-600 dark:border-white/15 dark:text-text-secondary dark:hover:border-primary-accent/50 dark:hover:text-primary-accent">
