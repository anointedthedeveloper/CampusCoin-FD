    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Every income and expense you&apos;ve logged.</p>
        </div>
          ...
          {(['all', 'income', 'expense'] as const).map((option) => (
            <button
              ...
              className={cn(
                ...
                typeFilter === option ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-600 hover:text-gray-900',
              )}
            >
              {option}
            </button>
          ...
          <div className="divide-y divide-gray-100">
            {transactions.map((txn) => (
              <div key={txn.id} className="flex items-center gap-3 px-5 py-4">
                ...
                >
                  <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-full', txn.type === 'income' ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-600')}>
                    <Receipt className="h-4 w-4" />
                  </span>
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-gray-900">{txn.description || categoryNameFor(txn.categoryId)}</p>
                    <p className="text-xs text-gray-500">{txn.type === 'income' ? 'Income' : categoryNameFor(txn.categoryId)} · {formatDate(txn.occurredAt)}</p>
                  </div>
                </Link>
                <span className={cn('shrink-0 text-sm font-semibold', txn.type === 'income' ? 'text-brand-600' : 'text-gray-900')}>
                  {txn.type === 'income' ? '+' : '-'}{formatCurrency(txn.amount, DEFAULT_CURRENCY)}
                </span>
                <button type="button" onClick={() => void handleDelete(txn.id)} className="shrink-0 rounded-lg p-1.5 text-gray-300 transition-colors duration-200 hover:bg-red-50 hover:text-red-600" aria-label="Delete transaction">
